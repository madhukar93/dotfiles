...
set -q fish_tmux_new_window_session || set fish_tmux_new_window_session true
...
function _fish_tmux_plugin_run
    ...
    else if test $fish_tmux_new_window_session && tmux has-session && test (count (tmux list-clients)) -gt 0
        exec $tmux_cmd -D new-session -t 0
    else
